...
public class AddClassDialogController2 {
	...
	// private ObservableList<BigClass> biglistTb =
	...
	private void smallAddAction() {
		...
		if (smallBtn.getText() == "수정") {
			if (tfComfrimField1()) {
				...
				alert.setTitle(null);
				alert.setHeaderText(null);
				alert.setContentText(name + ": 소분류 수정 되었습니다.");
				alert.showAndWait();
			...
			if (tfComfrimField1()) {
				...
				alert.setTitle(null);
				alert.setHeaderText(null);
				alert.setContentText(name + " : 소분류 추가 되었습니다.");
				alert.showAndWait();
